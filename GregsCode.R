...
wrapZ <- function(j) {
  ...
  Zj = matrix(NA,n,1)   # Z = A*Th - (b + d)
  eta  	<- theta%*%t(t(A[j,]))  # A*Th = eta
  bd	<- matrix(b[j]+d[j,],n,ncat-1,byrow=TRUE) # b+d
  hold <- sweep(bd,1,eta,"-")  # b+d - eta = -Z
  pp <- cbind(-Inf,hold,Inf)   # -Z
  ...
  pL <- pnorm( pp[indL] ) ; 	pU <- pnorm( pp[indU] )  #  pnorm(-Z)
  
  Zj <- eta + qnorm( pL + U*(pU - pL)  ) # Shift Z between boundaries... but Eta is already in Z?
  c(Zj)
}
...
meanD <- 0; meanB <- 0; covZ <- 0; plot(1,1); 
oldvec <- rep(0,Q)
Aold2 <- matrix(0,m,Q)
