FactorReconstruction<-function(condition,repl=NA,all.reps=T,
                               basedir="~/ParSAEM/SAEM_IRT/",
                               ThetaFix=FALSE,ContoursOnly=TRUE,...) {
    ...
    for (q in 1:sim.list[[d]]$Q) {
      ...
      if (sim.list[[d]]$Q==1) {
        ContourPlot(var1 = gThat[[paste0("T",q)]],var2 = That[[paste0("T",q)]],
                    ...
                    ylim=1.02*range(c(gThat[[paste0("T",q)]],That[[paste0("T",q)]])),
                    xlim=1.02*range(c(gThat[[paste0("T",q)]],That[[paste0("T",q)]])),...)
      } else {
        ContourPlot(var1 = gThat[[paste0("T",q)]],var2 = FThat[[paste0("T",q)]],
                    xlab=ptx,ylab=pty,main=sTitle,
                    ylim=1.02*range(c(gThat[[paste0("T",q)]],FThat[[paste0("T",q)]])),
                    xlim=1.02*range(c(gThat[[paste0("T",q)]],FThat[[paste0("T",q)]])),...)
      }
      treg<-lm(FThat[[paste0("T",q)]]~gThat[[paste0("T",q)]])
      print(summary(treg))
      abline(a=0,b=1,lwd=2,lty=3)
      abline(treg,lwd=2,col=2)
      lbl<-paste("=",paste(paste(apaformat(signif(coef(treg),digits=2)),c("+","*")),collapse=" "))
      text(2.5,-1,
           bquote(hat(theta)~.(lbl)~theta))
