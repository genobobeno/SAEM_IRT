TWSplitSC<-function(d,extra.dimensions=FALSE,E=10,tw=0.999,...) {
  ...
  for (di in d) {
    ...
    if (sum(d %in% c("S1","S2","S3"))>0 & di==d[1]) {
      ...
      # pBG<-c(0,3)
      pCH<-c(22,23,24,21,25)
      pCH.s<-c(1,1.2,1,1.2,1)
      pBG<-c(0,3)
    ...
    Q<-2
    sTitle<-eval(parse(text=paste0("expression('Condition ",gsub('S','',di)," : Largest'~",E,"~'Eigenvalues of'~S[2])")))
    ...
    if (di %in% c("S1","S2","S3")) {
      ...
      legend("topright",legend = ltext,text.width = max(strwidth(ltext))*1.02,
             lty=c(1,1,1,2),pch=c(pCH,NA),pt.cex=c(pCH.s,NA))
      ...
      strwidth(ltext)
      legend("topright",legend = ltext,text.width = max(strwidth(ltext))*1.02,
             lty=c(1,1,1,1,1,2),pch=c(pCH,NA),pt.cex=c(pCH.s,NA))
    ...
    Q<-2
    labNames <- c(paste0("Condition ",gsub('S','',di)," : Largest ",E," Eigenvalues of"),"cor(",")")
    sTitle<-bquote(.(labNames[1])~.(labNames[2])*S[2]*.(labNames[3]))
    #sTitle<-eval(parse(text=paste0("expression('Condition ",gsub('S','',di)," : Largest'~",E,"~'Eigenvalues of'~xlab))")))
    # if (di %in% c("S1","S2","S3","S5","S7","S9")) {
      plot(Q:E,c(0.7,gEV$values[Q:(E-1)]),type="n",main=sTitle,xlab="number of factors",ylab="estimators",lty=1,pch=1)
    # } else {
    #   plot(Q:E,log(c(0.7,gEV$values[Q:(E-1)])),type="n",main=sTitle,xlab="number of factors",ylab="log(estimators)",lty=1,pch=1)
    ...
    tw.exp<-eval(parse(text=paste0('expression(lambda["P=',tw,'"]("TW"[1]))')))
    if (di %in% c("S1","S2","S3")) {
      ltext =  c(#expression(lambda["Q=1"]("S"[2])),
                 expression(lambda["Q=1"]("cor"("S"[2]))),
                 #expression(lambda["Q=2"]("S"[2])),
                 expression(lambda["Q=2"]("cor"("S"[2]))),
                 #expression(lambda["Q=5"]("S"[2])),
                 expression(lambda["Q=5"]("cor"("S"[2]))),
                 tw.exp)
      strwidth(ltext)
      legend("topright",legend = ltext,text.width = max(strwidth(ltext))*1.02,
             lty=c(4,4,4,2),pch=c(pCH,NA),pt.cex=c(pCH.s,NA))
      #  lty=c(1,4,1,4,1,4,2),pch=c(0,0,2,2,5,5,NA))
    } else if (di %in% c("S4","S5")) {
      ltext =  c(#expression(lambda["Q=1"]("S"[2])),
                 expression(lambda["Q=1"]("cor"("S"[2]))),
                 #expression(lambda["Q=2"]("S"[2])),
                 expression(lambda["Q=2"]("cor"("S"[2]))),
                 #expression(lambda["Q=3"]("S"[2])),
                 expression(lambda["Q=3"]("cor"("S"[2]))),
                 #expression(lambda["Q=4"]("S"[2])),
                 expression(lambda["Q=4"]("cor"("S"[2]))),
                 #expression(lambda["Q=5"]("S"[2])),
                 expression(lambda["Q=5"]("cor"("S"[2]))),
                 tw.exp)
      strwidth(ltext)
      legend("topright",legend = ltext,text.width = max(strwidth(ltext))*1.02,
             lty=c(4,4,4,4,4,2),pch=c(pCH,NA),pt.cex=c(pCH.s,NA))
      #lty=c(1,4,1,4,1,4,1,4,1,4,2),pch=c(rep(pCH,c(2,2,2,2,2)),NA),pt.cex=c(rep(pCH.s,c(2,2,2,2,2)),NA))
    } else if (di %in% c("S6","S7")) {
      ltext =  c(#expression(lambda["Q=3"]("S"[2])),
                 expression(lambda["Q=3"]("cor"("S"[2]))),
                 #expression(lambda["Q=4"]("S"[2])),
                 expression(lambda["Q=4"]("cor"("S"[2]))),
                 #expression(lambda["Q=5"]("S"[2])),
                 expression(lambda["Q=5"]("cor"("S"[2]))),
                 #expression(lambda["Q=6"]("S"[2])),
                 expression(lambda["Q=6"]("cor"("S"[2]))),
                 #expression(lambda["Q=7"]("S"[2])),
                 expression(lambda["Q=7"]("cor"("S"[2]))),
                 tw.exp)
      strwidth(ltext)
      legend("topright",legend = ltext,text.width = max(strwidth(ltext))*1.02,
             lty=c(4,4,4,4,4,2),pch=c(pCH,NA),pt.cex=c(pCH.s,NA))
      ...
      ltext =  c(#expression(lambda["Q=8"]("S"[2])),
                 expression(lambda["Q=8"]("cor"("S"[2]))),
                 #expression(lambda["Q=9"]("S"[2])),
                 expression(lambda["Q=9"]("cor"("S"[2]))),
                 #expression(lambda["Q=10"]("S"[2])),
                 expression(lambda["Q=10"]("cor"("S"[2]))),
                 #expression(lambda["Q=11"]("S"[2])),
                 expression(lambda["Q=11"]("cor"("S"[2]))),
                 #expression(lambda["Q=12"]("S"[2])),
                 expression(lambda["Q=12"]("cor"("S"[2]))),
                 tw.exp)
      strwidth(ltext)
      legend("topright",legend = ltext,text.width = max(strwidth(ltext))*1.02,
             lty=c(4,4,4,4,4,2),pch=c(pCH,NA),pt.cex=c(pCH.s,NA))
