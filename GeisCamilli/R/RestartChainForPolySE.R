RestartChainForPolySE <- function(FitDATA,IT=2000,estgain=NA,thinA=NA,thinB=NA,TargetA=TRUE,rmethod=NA) {
  ...
  for (i in 1:IT) {
    ...
    if (TargetA) {
      ...
      } else if (!is.na(FitDATA$settings$rmethod)) {
        Target <- mat.or.vec(J,settings$Adim) #FitDATA$xi[,1:settings$Adim]
        Target[abs(Target)>0.15] <- NA
        Target <- matrix(Target,J,settings$Adim)
        RotT <- targetQ(Anew,Tmat=diag(settings$Adim),Target=Target,normalize=TRUE,eps=1e-4, maxit=1000)
        if (sum(colSums(RotT$loadings)<0)>0) {
          for (q in which(colSums(RotT$loadings)<0)) {
            RotT$loadings[,q]<-(-1)*RotT$loadings[,q]
          }
