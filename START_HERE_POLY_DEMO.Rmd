...
cor((MyFirstFit$That[,1:10]%*%MyFirstFit$AR$Th),GEN.DATA$THETA)

# Rotate latent factor estimates, and plot them and their residuals
RTheta<-(MyFirstFit$That[,1:10]%*%MyFirstFit$AR$Th)
par(mfrow=c(5,2))
for (i in 1:10) {plot(GEN.DATA$THETA[,i],RTheta[,i])}
for (i in 1:10) {plot(density(GEN.DATA$THETA[,i]-RTheta[,i]))}
```
