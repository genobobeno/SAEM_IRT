...
TracyWidom<-function(gamma,p=NA,n=NA) {
...
mpd1<-MP_Sample(gamma=0.01,p=100,samples=500)
mpd2<-MP_Sample(gamma=0.1,p=100,samples=500)
mpd3<-MP_Sample(gamma=0.01,p=10,samples=500)
mpd4<-MP_Sample(gamma=0.1,p=10,samples=500)

par(mfrow=c(2,2))
bks<-seq(0,2.5,length.out = 125)
hist(mpd3$ev,freq = FALSE,col = 2,breaks = bks,main=expression(paste("500 samples : ",italic(J)," = 10, ",italic(N)," = 1000")),xlab=expression(lambda))
lines(mpd3$x.mp,mpd3$y.mp,lwd=2,col=4)
legend("topright",legend = expression(italic(P)[MP] ~ " : " ~ italic(c) ~ "= 0.01"),lty=1,lwd=2,col=4)
hist(mpd4$ev,freq = FALSE,col = 2,breaks = bks,main=expression(paste("500 samples : ",italic(J)," = 10, ",italic(N)," = 100")),xlab=expression(lambda),ylim=c(0,3.3))
lines(mpd4$x.mp,mpd4$y.mp,lwd=2,col=4)
legend("topright",legend = expression(italic(P)[MP] ~ " : " ~ italic(c) ~ "= 0.1"),lty=1,lwd=2,col=4)
hist(mpd1$ev,freq = FALSE,col = 2,breaks = bks,main=expression(paste("500 samples : ",italic(J)," = 100, ",italic(N)," = 10000")),xlab=expression(lambda))
lines(mpd1$x.mp,mpd1$y.mp,lwd=2,col=4)
legend("topright",legend = expression(italic(P)[MP] ~ " : " ~ italic(c) ~ "= 0.01"),lty=1,lwd=2,col=4)
hist(mpd2$ev,freq = FALSE,col = 2,breaks = bks,main=expression(paste("500 samples : ",italic(J)," = 100, ",italic(N)," = 1000")),xlab=expression(lambda),ylim=c(0,3.3))
